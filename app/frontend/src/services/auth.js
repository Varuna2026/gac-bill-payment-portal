import { P2_LOGIN_USERS } from '../data/p2LoginUsers'

const storageKey = 'p2v2_auth_session_v4'
const P2_USERS = P2_LOGIN_USERS
...
export async function loginWithUsername(username, credential) {
  const id = username.trim()
  if (!id || !credential) throw new Error('Enter ID and Password.')
  const user = P2_USERS.find(x => x.username.toLowerCase() === id.toLowerCase())
  const valid = user?.test_login ? credential === user.username : await passwordHash(credential) === P2_PASSWORD_HASH
  if (!user || !valid) throw new Error('Invalid ID or Password.')
