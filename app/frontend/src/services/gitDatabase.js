...
function assertTransition(row,v){const role=sessionProfile()?.role;if(!role||role==='ADMIN')return;const stage=row.current_stage,status=row.current_status,ns=v.current_stage,nst=v.current_status;const remarks=String(v.latest_remarks??'').trim();const fail=m=>{throw Error(`Workflow gate failed: ${m}`)};const stageFor={VENDOR:'VENDOR',WH:'WH',GAC_COMPLIANCE:'GAC_COMPLIANCE',GAC_PO:'GAC_PO',ACCOUNTS:'ACCOUNTS',CBO_OFFICE:'CBO_OFFICE',CBO_OFFICER:'CBO_OFFICER'};if(stage!==stageFor[role])fail(`only ${role} may action this stage`);if(nst==='REJECTED'&&!remarks)fail('Remarks are mandatory when an invoice is rejected');const ok=(s,st,d,ds)=>stage===s&&status===st&&ns===d&&nst===ds
