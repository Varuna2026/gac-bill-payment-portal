...
function readDb() { try { const saved = localStorage.getItem(DB_KEY); return saved ? JSON.parse(saved) : initialState() } catch { return initialState() } }
...
function assertWorkflowTransition(row, values) {
  const user = sessionProfile(); const role = user?.role
  ...
  if (queryReturnReject) {
    const destinations = { WH:['VENDOR'], GAC_COMPLIANCE:['VENDOR','WH'], GAC_PO:['VENDOR','WH'], ACCOUNTS:['GAC_PO'], CBO_OFFICE:['ACCOUNTS','GAC_PO','WH','VENDOR'], CBO_OFFICER:['CBO_OFFICE','ACCOUNTS','GAC_PO','WH','VENDOR'] }[role] || []
    if (!destinations.includes(ns)) fail(`invalid correction/rejection destination ${ns}`)
    return
  }
...
export async function insertRows(table, rows) {
  ...
  for (const row of input) {
    ...
    if (table === 'pr_po_utr') {
      ...
      if (row.pr_number && !['WH','ADMIN'].includes(role)) throw new Error('Only Warehouse can map PR.')
      if (row.po_number && !['GAC_PO','ADMIN'].includes(role)) throw new Error('Only GAC PO can map PO.')
      if (row.utr_number && !['ACCOUNTS','ADMIN'].includes(role)) throw new Error('Only final Accounts can map UTR.')
    }
  }
...
export async function updateRows(table, query, values) {
  ...
  if (table === 'invoice_records' && (values.current_stage || values.current_status)) { if (hits.length !== 1) throw new Error('Workflow update must target exactly one invoice.'); assertWorkflowTransition(hits[0], values) }
  const matched = []
  db[table] = db[table].map(row => { if (!filterRows([row], query).length) return row; const changed = { ...row, ...values }; matched.push(changed); return changed })
