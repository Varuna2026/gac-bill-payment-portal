...
export async function uploadStorage(path, file, token) {
 if (!supabaseConfigured) throw new Error('Supabase is not configured')
 const response = await fetch(`${SUPABASE_URL}/storage/v1/object/invoice-documents/${path}`, { method: 'POST', headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${token || SUPABASE_KEY}`, 'Content-Type': file.type || 'application/octet-stream', 'x-upsert': 'false' }, body: file })
 if (!response.ok) { const text = await response.text(); throw new Error(text || `Storage upload failed: ${response.status}`) }
