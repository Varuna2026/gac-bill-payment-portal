...
function complianceHistory(history,id){
  ...
  if(lastQuery){
    ...
    const next=h.find(x=>new Date(x.action_at||0).getTime()>qAt&&((x.action==='Vendor Resubmit'&&x.from_stage==='VENDOR')||(x.action==='Resubmit to GAC Compliance'&&x.from_stage==='WH')))
...
export default function P2Reports({data,role,history=[]}){
  ...
  const enriched=useMemo(()=>scopeRows.map(x=>({...x,_compliance:complianceHistory(history,x.id)})),[scopeRows,history])
  const vals=k=>[...new Set(enriched.map(x=>k==='month'?monthOf(x):k==='compliance'?x._compliance.status:x[k]).filter(Boolean))].sort().reverse()
  const rows=useMemo(()=>enriched.filter(x=>{const m=monthOf(x);return (f.month==='All'||m===f.month)&&(f.company==='All'||x.company===f.company)&&(f.warehouse==='All'||x.warehouse===f.warehouse)&&(f.project==='All'||x.project_location===f.project)&&(f.vendor==='All'||x.vendor===f.vendor)&&(f.service==='All'||x.service_type===f.service)&&(f.compliance==='All'||x._compliance.status===f.compliance)}),[enriched,f])
  const reportRows=rows.map((x,i)=>({sl:i+1,month:monthOf(x),company:x.company,warehouse:x.warehouse,project:x.project_location,vendor:x.vendor,service:x.service_type,invoice:x.inv_no,taxable:x.taxable_amt??x.taxable_amount??x.amount_before_gst??'',incl:x.amt_incl_gst??x.amount_incl_gst??'',billRec:x._compliance.billRec,status:x._compliance.status,query:x._compliance.query,queryResolved:x._compliance.queryResolved,navision:x._compliance.navision}))
  const download=()=>{const headers=['Sl No','Month','Company','Warehouse','Project','Vendor','Service Type','Invoice No','Taxable Amt','Amt (Incl GST)','Bill Rec @ HO','Compliance Status','Query Details','Query resolved on','H/O for Navision (Date)'];const body=reportRows.map(r=>[r.sl,monthLabel(r.month),r.company,r.warehouse,r.project,r.vendor,r.service,r.invoice,r.taxable,r.incl,dateOnly(r.billRec),r.status,r.query,dateOnly(r.queryResolved),dateOnly(r.navision)]);const html=`<html><head><meta charset="utf-8"></head><body><table><tr>${headers.map(h=>`<th>${esc(h)}</th>`).join('')}</tr>${body.map(r=>`<tr>${r.map(v=>`<td>${esc(v)}</td>`).join('')}</tr>`).join('')}</table></body></html>`;const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([html],{type:'application/vnd.ms-excel'}));a.download='GAC_Vendor_Bill_Payment_Tracker.xls';a.click()}
